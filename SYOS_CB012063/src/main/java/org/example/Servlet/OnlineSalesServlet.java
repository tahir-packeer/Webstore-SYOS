...
public class OnlineSalesServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            try {
                ...
                if (customer != null) {
                    System.out.println("Online customer found: " + customer.getName() + " (ID: " + customer.getId() + ")");
                }
            } catch (Exception e) {
                System.out.println("Customer lookup in online_customers table failed: " + e.getMessage());
            }
            
            // Fallback: check regular customer table (in case customer was registered in-store first)
            if (customer == null) {
                try {
                    customer = customerController.get_Customer_from_contactNumber(customerIdentifier);
                    if (customer != null) {
                        System.out.println("Regular customer found (cross-lookup): " + customer.getName() + " (ID: " + customer.getId() + ")");
                    }
                } catch (Exception e) {
            ...
            if (customer == null) {
                try {
                    ...
                    if (customer != null) {
                        System.out.println("Customer found by ID: " + customer.getName() + " (ID: " + customer.getId() + ")");
                    }
            ...
            if (customer == null) {
                resp.setStatus(400);
                resp.getWriter().write("{\"error\":\"Customer not found with identifier: " + customerIdentifier + ". Please ensure you are logged in with a valid account.\"}");
                return;
            }
            ...
            for (int i = 0; i < itemsArr.length(); i++) {
                ...
                if (availableQuantity < quantity) {
                    resp.setStatus(400);
                    resp.getWriter().write("{\"error\":\"Insufficient stock for " + item.getName() + ". Available: " + availableQuantity + ", Requested: " + quantity + "\"}");
                    return;
                }
            }
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        resp.setContentType("application/json");
    try (Connection conn = org.example.Database.DatabaseConnection.getInstance().connect()) {
            PreparedStatement ps = conn.prepareStatement("SELECT * FROM bill WHERE transactionType = 'ONLINE' ORDER BY billDate DESC LIMIT 100");
