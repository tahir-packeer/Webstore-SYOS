...
public class CustomerController {
            ...
            throws SQLException, ClassNotFoundException
    {
        ...
        try {
            var statement = connection.prepareStatement("select * from customer where contact_number = ?");
            statement.setString(1, contactNumber);
            var resultSet = statement.executeQuery();
            if (resultSet.next()) {
                String customerName = resultSet.getString("name");
                String customerContact = resultSet.getString("contact_number");
    ...
    public Customer get_OnlineCustomer_from_contactNumber
            (String contactNumber)
            throws SQLException, ClassNotFoundException
    {
        DatabaseConnection databaseConnection = DatabaseConnection.getInstance();
